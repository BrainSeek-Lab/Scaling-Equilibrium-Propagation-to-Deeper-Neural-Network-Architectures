...
class Monitor:
    ...
    def _build_series(self):
        ...
        # Handle evaluation-only mode gracefully
        if self._trainer is not None:
            ...
            for stat in stats_train_0:
                self._add_statistic(stat, train=True)
            for stat in stats_train_1:
                self._add_statistic(stat, train=True, list_idx=1)

        # Stats for evaluator (evaluation phase)
    ...
    def evaluate_only(self, verbose=False):
        """Runs evaluation using the evaluator and logs the stats."""
        if self._evaluator is None:
            raise ValueError("Evaluator not set for Monitor — cannot run evaluation.")
        
        if verbose:
            print("[Monitor] Running evaluation-only pass...")
    ...
    def _update_summary_writer(self):
        ...
        for series in self._series:
            if series.name:
                ...
                # self._writer.add_scalar(series.name, value, self._epoch)  # TensorBoard
                log_dict[series.name] = value  # wandb
        if wandb.run is not None:
            wandb.log(log_dict, step=self._epoch)
        # FIXME
...
class Optimizer(torch.optim.SGD):
    ...
    def __init__(self, energy_fn, cost_fn, learning_rates, momentum=0, weight_decay=0):
        ...
        torch.optim.SGD.__init__(self, params, lr=0.1, momentum=momentum, weight_decay=weight_decay,nesterov=True)

    def __str__(self):
        return 'SGD -- initial learning rates = {}, momentum={}, weight_decay={}'.format(self._learning_rates, self._momentum, self._weight_decay)
    
class adamOptimizer(torch.optim.AdamW):
    def __init__(self, energy_fn, cost_fn, learning_rates, weight_decay=0.01):
        self._learning_rates = learning_rates
        self._weight_decay = weight_decay

        # Combine all parameters and assign learning rates
        params = energy_fn.params() + cost_fn.params()
        param_groups = [{"params": param.state, "lr": lr} for param, lr in zip(params, learning_rates)]

        # Initialize AdamW
        super().__init__(param_groups, weight_decay=weight_decay)

    def __str__(self):
        return 'adam -- initial learning rates = {},weight_decay={}'.format(self._learning_rates,  self._weight_decay)
