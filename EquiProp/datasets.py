...
def load_cifar100(normalize=True):
    ...
    train_transform = torchvision.transforms.Compose([
        ...
        torchvision.transforms.RandAugment(num_ops=4, magnitude=9),
        torchvision.transforms.RandomCrop(size=[32,32], padding=4, padding_mode='edge'),
        torchvision.transforms.ToTensor(),
        final_transform
    ])
...
def load_dataset(dataset, normalize=True, augment_32x32=True):
    ...
    if dataset == 'FashionMNIST': return load_fashion_mnist(normalize, augment_32x32)
    elif dataset == 'CIFAR10': return load_cifar10(normalize)
    elif dataset == 'CIFAR100': return load_cifar100(normalize)
    else: raise ValueError("expected  'FashionMNIST', 'CIFAR10' or 'CIFAR100' but got {}".format(dataset))


def load_dataloaders(dataset, batch_size, augment_32x32=True, normalize=True):
    ...
    training_loader = torch.utils.data.DataLoader(training_data, batch_size=batch_size, shuffle=True, num_workers=16,pin_memory=True)
    test_data = IndexedDataset(test_data)
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=batch_size, shuffle=False, num_workers=16,pin_memory=True)
