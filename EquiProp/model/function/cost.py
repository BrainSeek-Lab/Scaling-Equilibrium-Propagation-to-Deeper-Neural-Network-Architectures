...
class CrossEntropy(CostFunction, QFunction):
    """CrossEntropy cost function for classification."""

    def __init__(self, layer):
        self._layer = layer
        num_classes = layer.state.shape[1]
        CostFunction.__init__(self, num_classes)
        Function.__init__(self, [layer], [])

    def eval(self):
        """Returns the cross-entropy loss for each sample."""
        logits = self._layer.state                      # raw logits
        log_probs = F.log_softmax(logits, dim=1)        # log-softmax
        target = self._target                           # one-hot
    ...
    def grad_layer_fn(self, layer):
        return {self._layer: self._grad_layer}[layer]

    def a_coef_fn(self, layer):
        return {self._layer: self._a_coef}[layer]

    def b_coef_fn(self, layer):
        return {self._layer: self._b_coef}[layer]

    def _grad_layer(self):
        """Gradient wrt logits = softmax(logits) - target"""
        logits = self._layer.state
        probs = F.softmax(logits, dim=1)
        return probs - self._target
